...
- 下一步顺序：以 `review-output-20260910-v1/human-review-checklist.corrected.json` 为输入执行 `create-review-artifact → create-review-set → pin-review-manifest → convert-formal → quota → freeze-selection → verify-selection-chain` → 用 `scripts/prepare_formal_intent.py` 生成正式 config + intent manifest（GA hy3、`HY3_TIMEOUT_SECONDS=600` 且与 `--timeout-seconds` 相等、预算 500）→ 正式基准（formal=true）→ 报告/演示；differential 判题扩展可同批或另批。单题 smoke 与三题 MVP 均不可外推为正式性能或资格。

...
The unpersisted pre-release `1.0` draft has no migration obligation. The reviewed `1.1` contract is the first frozen artifact version; after this freeze, every breaking change requires a new version and an explicit reader or migration path before implementation.
